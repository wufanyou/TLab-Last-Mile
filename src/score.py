    ...
    if input_type == "proposed" or input_type == "actual":
        for route in file:
            if type(file[route]) != dict or len(file[route]) != 1:
                raise JSONDecodeError(
                    "Improper route in {}. Each route ID must map to a dictionary with a single key.".format(
                        filepath
                    )
                )
            ...
            if type(file[route][input_type]) != dict:
                raise JSONDecodeError(
                    "Improper route in {}. Each sequence must be in the form of a dictionary.".format(
                        filepath
                    )
                )
    ...
    if input_type == "costs":
        for route in file:
            if type(file[route]) != dict:
                raise JSONDecodeError(
                    "Improper matrix in {}. Each cost matrix must be a dictionary.".format(
                        filepath
                    )
                )
            for origin in file[route]:
                ...
                for dest in file[route][origin]:
                    ...
                    if not (
                        type(file[route][origin][dest]) == float
                        or type(file[route][origin][dest]) == int
                    ):
                        raise JSONDecodeError(
                            "Improper time in {}. Every travel time must be a float or int.".format(
                                filepath
                            )
                        )
...
def evaluate(
    ...
    **kwargs :
    ...
    for route in tqdm(actual_routes):
